...
async function analyzeWithGPT4Vision(file, reportType) {
  ...
  try {
    ...
    const visionModel = "gpt-4o"; // Updated to use gpt-4o which has better vision and PDF support
      ...
      // Build content array - support both images and PDFs
      const contentArray = [
        {
          type: "text",
          text: analysisPrompt
...
【必須抽出項目（5つの重要指標）】
以下の5つの指標を必ず具体的な数値で抽出してください：
1. FCR（総収益率）: [具体的な数値]%
2. K%（ローン定数）: [具体的な数値]%
3. CCR（自己資金配当率、キャッシュオンキャッシュ）: [具体的な数値]% ※特に重要、必ず抽出
4. DCR（借入金償還余裕率）: [具体的な数値]倍
5. BER（損益分岐入居率）: [具体的な数値]%

【IRR指標（表形式での出力必須）】
以下の表形式でIRR指標を必ず抽出してください：
