...
class TiptapEditorClean {
    ...
    /**
     * Load Tiptap packages from node_modules
     */
    async loadTiptapPackages() {
        try {
            ...
            const { TiptapMarkdown } = await import('/node_modules/tiptap-markdown/dist/index.js');
            const { Table } = await import('/node_modules/@tiptap/extension-table/dist/index.js');
            const { TableRow } = await import('/node_modules/@tiptap/extension-table-row/dist/index.js');
            const { TableCell } = await import('/node_modules/@tiptap/extension-table-cell/dist/index.js');
            const { TableHeader } = await import('/node_modules/@tiptap/extension-table-header/dist/index.js');
            ...
            console.log('[TiptapEditorClean] Tiptap packages loaded successfully');
            return true;
        } catch (error) {
            console.error('[TiptapEditorClean] Failed to load Tiptap packages:', error);
            ...
            try {
                ...
                console.log('[TiptapEditorClean] Tiptap packages loaded via alternative import');
                return true;
            } catch (altError) {
                throw new Error('Tiptap packages not available. Please run: npm install @tiptap/core @tiptap/starter-kit tiptap-markdown @tiptap/extension-table @tiptap/extension-table-row @tiptap/extension-table-cell @tiptap/extension-table-header');
            }
        }
    }
    ...
    setContent(content, format = 'markdown') {
        ...
        try {
            if (format === 'markdown') {
                // TiptapMarkdown should parse Markdown automatically when setting content
                // Use setContent with the Markdown string directly
