...
class TiptapEditorClean {
    ...
    async loadTiptapPackages() {
        ...
        try {
            ...
            try {
                console.log('[TiptapEditorClean] Loading Tiptap packages from esm.sh CDN (v3.10.1)...');
                
                const cdnBase = 'https://esm.sh';
                
                // Import all required Tiptap extensions from CDN
                // Use v3.10.1 to match package.json versions
                const { Editor } = await import(`${cdnBase}/@tiptap/core@3.10.1`);
                const { default: StarterKit } = await import(`${cdnBase}/@tiptap/starter-kit@3.10.1`);
                const { TiptapMarkdown } = await import(`${cdnBase}/tiptap-markdown@0.9.0`);
                const { Table } = await import(`${cdnBase}/@tiptap/extension-table@3.10.1`);
                const { TableRow } = await import(`${cdnBase}/@tiptap/extension-table-row@3.10.1`);
                const { TableCell } = await import(`${cdnBase}/@tiptap/extension-table-cell@3.10.1`);
                const { TableHeader } = await import(`${cdnBase}/@tiptap/extension-table-header@3.10.1`);
