...
class TabNavigation {
    ...
    initializeEditorTab() {
        ...
        if (!editorContent) {
            this.handleComponentError('RichTextEditor', 'Container element not found');
            return;
        }
        ...
        try {
            ...
            if (this.contentState) {
                ...
                if (editedContent.content) {
                    this.safeSetContent(this.richTextEditor, editedContent.content, 'RichTextEditor');
                } else {
                    ...
                    if (originalContent.content) {
                        const htmlContent = this.convertMarkdownToEditableHTML(originalContent.content);
                        this.safeSetContent(this.richTextEditor, htmlContent, 'RichTextEditor');
                    }
                }
            }
            
            // Mark as successfully initialized
            this.componentErrors.delete('RichTextEditor');
            
        } catch (error) {
            this.handleComponentError('RichTextEditor', error);
            this.showComponentFallback(editorContent, 'RichTextEditor', 'リッチテキストエディターの初期化に失敗しました');
        }
    }
    ...
    convertMarkdownToEditableHTML(markdown) {
        ...
        // Fallback: Simple markdown to HTML conversion for editing
        // Note: Fixed regex patterns - removed escaped backslashes
        let html = markdown
            ...
            // Use a simpler approach: replace remaining single * that aren't part of **
            .replace(/\*([^*\n]+?)\*/g, '<em>$1</em>')
    ...
    safeSetContent(component, content, componentName) {
        try {
            if (component && component.setContent) {
