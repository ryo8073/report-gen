    ...
    initializeEditorTab() {
        ...
        try {
            if (!this.richTextEditor) {
                // Use clean TiptapEditorClean for proper WYSIWYG editing
                // This is the new zero-based rebuild following Tiptap design principles
                if (typeof TiptapEditorClean !== 'undefined') {
                    this.richTextEditor = new TiptapEditorClean(editorContent, {
                        enableToolbar: true,
                        enableAutoSave: this.options.enableAutoSave,
                        autoSaveInterval: this.options.autoSaveInterval,
                        placeholder: 'レポートを編集してください...',
                        onContentChange: (htmlContent, textContent) => {
                            // HTML content change callback
                            this.safeHandleEditorContentChange('', textContent);
                        },
                        onMarkdownChange: (markdownContent) => {
                            // Markdown content change callback - this is the primary callback
                ...
                } else if (typeof TiptapWYSIWYGEditorReal !== 'undefined') {
            ...
            const setInitialContent = () => {
                if (this.contentState && this.richTextEditor) {
                    ...
                    if (editedContent.content) {
                        ...
                        if (this.richTextEditor instanceof TiptapEditorClean) {
                            // Clean TiptapEditor accepts Markdown directly
                            this.richTextEditor.setContent(content, 'markdown');
                        } else if (this.richTextEditor instanceof TiptapWYSIWYGEditorReal || this.richTextEditor instanceof TiptapWYSIWYGEditor) {
                            this.richTextEditor.setContent(content, 'markdown');
                        } else {
