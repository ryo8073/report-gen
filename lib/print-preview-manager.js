...
class PrintPreviewManager {
    ...
    generatePages() {
        ...
        if (this.isMarkdownContent(this.content)) {
            htmlContent = this.convertMarkdownToHTML(this.content);
        }
    ...
    createPageElement(pageNumber) {
        ...
        let pageContent = this.getContentForPage(pageNumber);
        // Convert markdown to HTML if needed
        if (this.isMarkdownContent(pageContent)) {
            pageContent = this.convertMarkdownToHTML(pageContent);
        }
    ...
    isMarkdownContent(content) {
        ...
        return /^#+\s|^\*\*|^\*[^*]|^-\s|^\d+\.\s|```|`/.test(content.trim()) ||
               /\*\*.*?\*\*/.test(content) ||
               /^##/.test(content);
    }
    
    /**
     * Convert markdown to HTML using MarkdownRenderer if available
     */
    convertMarkdownToHTML(markdown) {
        if (!markdown || typeof markdown !== 'string') return '';
        
        try {
            // Use MarkdownRenderer if available (preferred method)
            if (typeof MarkdownRenderer !== 'undefined') {
                ...
                return renderer.render(markdown, true); // Use business formatting
            }
