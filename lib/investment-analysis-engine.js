...
class InvestmentAnalysisEngine {
    ...
    determineRecommendationLevel(metrics) {
        if (metrics.npv > 0) return '推奨';
        return '非推奨';
    }
    
    generateExecutiveSummary(metrics, leverageAnalysis, valuationAnalysis) {
        const gradeText = this.getInvestmentGradeText(valuationAnalysis.investmentGrade);
        return `投資分析結果: ${valuationAnalysis.recommendationLevel} (投資グレード: ${gradeText})`;
    }
    
    /**
     * Convert investment grade letter to Japanese text
     */
    getInvestmentGradeText(grade) {
        const gradeMap = {
            'A': '優良',
            'B': '良好',
            'C': '普通',
