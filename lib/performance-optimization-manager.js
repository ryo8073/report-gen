...
class PerformanceOptimizationManager {
    ...
    async lazyLoadLibraries(libraryType = 'all') {
        ...
        try {
            ...
            for (const library of librariesToLoad) {
                if (!this.loadedLibraries.has(library.name)) {
                    ...
                    if (this.loadingPromises.has(library.name)) {
                        ...
                        // Wrap loadLibrary to handle errors gracefully
                        const loadPromise = this.loadLibrary(library).catch(error => {
                            console.warn(`Failed to load library ${library.name}:`, error.message);
                            // Return false instead of throwing to allow other libraries to load
                            return false;
                        });
            ...
            // Use allSettled to allow some libraries to fail without breaking others
            ...
            if (failed > 0) {
                console.warn(`Lazy loaded ${successful}/${librariesToLoad.length} libraries (${failed} failed) in ${Math.round(loadTime)}ms`);
            } else {
    ...
    getLibrariesToLoad(libraryType) {
        const libraries = {
            ...
            word: [
                {
                    name: 'docx',
                    // Note: docx library is primarily an ES module and may not work as a global script
                    // Loading is optional - Word export will check for library availability when needed
                    url: 'https://unpkg.com/docx@8.5.0/build/index.js',
                    globalName: 'docx',
                    check: () => window.docx || window.docxjs,
                    optional: true // Mark as optional so failure doesn't break the app
                }
            ]
        };
    ...
    async loadLibrary(library) {
        return new Promise((resolve, reject) => {
            ...
            // If library is marked as optional, handle failures gracefully
            if (library.optional) {
                ...
                script.onload = () => {
                    ...
                    if (library.check()) {
                        this.loadedLibraries.add(library.name);
                        console.log(`Loaded optional library: ${library.name}`);
                        resolve();
                    } else {
                        // Optional library failed to initialize, but that's okay
                        console.warn(`Optional library ${library.name} loaded but not available as expected (this is normal for some libraries)`);
                        resolve(); // Resolve instead of reject for optional libraries
                    }
                };
                
                script.onerror = () => {
                    // For optional libraries, log warning but don't reject
                    console.warn(`Optional library ${library.name} failed to load from CDN (Word export may not work - library will be loaded on-demand if needed)`);
                    resolve(); // Resolve instead of reject
                };
                
                document.head.appendChild(script);
            } else {
                // Required libraries - fail if they can't load
                const script = document.createElement('script');
                script.src = library.url;
                script.async = true;
