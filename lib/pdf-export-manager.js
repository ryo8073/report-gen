    ...
    async addPageHeader(pageNumber, options, pageDims, margins) {
        ...
        if (headerText && typeof headerText === 'string' && headerText.trim()) {
            try {
                ...
                // Ensure coordinates are valid (positive numbers)
                const x = Math.max(0, margins.left);
                const y = Math.max(10, margins.top - 5); // Ensure Y is at least 10 from top
                this.pdf.text(headerText.trim(), x, y);
            } catch (error) {
                console.warn('Failed to add header text to PDF:', error);
                // Continue without header rather than failing entire export
            }
        }
