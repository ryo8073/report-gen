...
class EnhancedResultsLayout {
    ...
    setupContentEditingListeners() {
        // Initialize Tiptap editors (they handle their own listeners)
        // Listeners are set up in initializeEditView and initializeSplitView
        
        // Keep toolbar listeners for backward compatibility
        // Toolbar is now handled by TiptapEditorClean, but we keep these for any additional actions
    ...
    initializeEditView() {
        ...
        // Initialize TiptapEditorClean if not already done
        if (!this.contentEditor && typeof TiptapEditorClean !== 'undefined') {
            try {
                this.contentEditor = new TiptapEditorClean(editorContainer, {
                    ...
                    onContentChange: (htmlContent, textContent) => {
                        // HTML content change (less relevant, but keep for compatibility)
                    },
                ...
                this.setupTextareaFallback(editorContainer, 'content-editor');
            }
        } else if (this.contentEditor && this.content) {
    ...
    /**
     * Set up textarea fallback
     */
    setupTextareaFallback(container, id) {
        container.innerHTML = `<textarea id="${id}" class="content-textarea" placeholder="ここでレポートを編集できます..." aria-label="レポート内容の編集"></textarea>`;
        const textarea = container.querySelector(`#${id}`);
        if (textarea && this.content) {
            textarea.value = this.content;
            textarea.addEventListener('input', (e) => {
                this.handleContentChange(e.target.value, id === 'content-editor' ? 'edit' : 'split');
            });
        }
    }
    ...
    initializeSplitView() {
        ...
        // Initialize TiptapEditorClean for split view if not already done
        if (!this.splitEditor && typeof TiptapEditorClean !== 'undefined') {
            try {
                ...
                // Fallback to textarea if Tiptap fails
                this.setupTextareaFallback(editorContainer, 'split-editor');
            }
        } else if (this.splitEditor && this.content) {
            // Update existing editor content
            this.splitEditor.setContent(this.content, 'markdown');
        }
    ...
    setContent(content, metadata = {}) {
        ...
        if (this.contentEditor && typeof this.contentEditor.setContent === 'function') {
            ...
            // Fallback to textarea if Tiptap not initialized
            const contentEditor = this.container.querySelector('#content-editor');
            if (contentEditor) {
                contentEditor.value = this.content;
            }
        }
        ...
        if (this.splitEditor && typeof this.splitEditor.setContent === 'function') {
            ...
            if (splitEditor) {
                splitEditor.value = this.content;
            }
        }
    ...
    handleContentChange(newContent, source) {
        ...
        // Update other editors if needed
        if (source === 'edit') {
            // Update split editor
            if (this.splitEditor && typeof this.splitEditor.setContent === 'function') {
                ...
                const splitEditor = this.container.querySelector('#split-editor');
                if (splitEditor && splitEditor.value !== newContent) {
                    splitEditor.value = newContent;
                }
            }
            ...
            if (this.contentEditor && typeof this.contentEditor.setContent === 'function') {
                ...
                // Fallback to textarea
                const contentEditor = this.container.querySelector('#content-editor');
                if (contentEditor && contentEditor.value !== newContent) {
    ...
    setupEditorAccessibility() {
        const editors = this.container.querySelectorAll('#content-editor, #split-editor');
        ...
        editors.forEach(editor => {
            ...
            if (editor.id === 'content-editor') {
                editor.setAttribute('aria-label', 'メインエディター');
            } else if (editor.id === 'split-editor') {
                editor.setAttribute('aria-label', '分割表示エディター');
            }
            
            // Add description
            const descriptionId = `${editor.id}-description`;
            const description = document.createElement('div');
            description.id = descriptionId;
            description.className = 'sr-only';
            description.textContent = 'テキストエディターです。Ctrl+Bで太字、Ctrl+Iで斜体、Ctrl+Uで下線の書式設定ができます。';
            editor.parentNode.appendChild(description);
    ...
    getActiveEditor() {
        if (this.currentView === 'edit') {
            return this.container.querySelector('#content-editor');
        } else if (this.currentView === 'split') {
            return this.container.querySelector('#split-editor');
        }
        return null;
    }
    ...
    getEditorSource(editor) {
        if (editor.id === 'content-editor') return 'edit';
        if (editor.id === 'split-editor') return 'split';
        return 'unknown';
    }
    ...
    disableSplitSync() {
        ...
        if (splitEditor && this.splitSyncHandler) {
            splitEditor.removeEventListener('scroll', this.splitSyncHandler);
